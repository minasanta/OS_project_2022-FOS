...
int test_kmalloc_firstfit1()
{
	...
	//[3] Allocate again [test first fit]
	{
		...
		if((pf_calculate_free_frames() - freeDiskFrames) !=  0)  panic("Page file is changed while it's not expected to. (pages are wrongly allocated/de-allocated in PageFile)");
		if ((freeFrames - sys_calculate_free_frames()) != 256) panic("Wrong allocation: ",(freeFrames - sys_calculate_free_frames()));
...
{
	if(USE_KHEAP != 1)
		kpanic_into_prompt("USE_KHEAP = 0 & it shall be 1. Go to 'inc/memlayout.h' and set USE_KHEAP by 1. Then, repeat the test again.");
	if(STATIC_MEMBLOCK_ALLOC != 0)
		kpanic_into_prompt("STATIC_MEMBLOCK_ALLOC = 1 & it shall be 0. Go to 'inc/dynamic_allocator.h' and set STATIC_MEMBLOCK_ALLOC by 0. Then, repeat the test again.");

	//Check MAX_MEM_BLOCK_CNT
	if(MAX_MEM_BLOCK_CNT != ((0xFFFFF000-0xF6000000)/4096))
	{
		kpanic_into_prompt("Wrong initialize: MAX_MEM_BLOCK_CNT is not set with the correct size of the array");
	}

	//Check number of nodes in AvailableMemBlocksList
	if (LIST_SIZE(&(AvailableMemBlocksList)) != MAX_MEM_BLOCK_CNT-1)
	{
		kpanic_into_prompt("Wrong initialize: Wrong size for the AvailableMemBlocksList");
	}

	//Check number of nodes in AllocMemBlocksList
	if (LIST_SIZE(&(AllocMemBlocksList)) != 0)
	{
		kpanic_into_prompt("Wrong initialize: Wrong size for the AllocMemBlocksList");
	}

	//Check number of nodes in FreeMemBlocksList
	if (LIST_SIZE(&(FreeMemBlocksList)) != 1)
	{
		kpanic_into_prompt("Wrong initialize: Wrong size for the FreeMemBlocksList");
	}
	...
	{
		kpanic_into_prompt("Wrong initialize: Wrong content for the FreeMemBlocksList.");
	}

	//Check number of disk and memory frames
	if ((freeDiskFrames_after - freeDiskFrames_before) != 0) kpanic_into_prompt("Page file is changed while it's not expected to. (pages are wrongly allocated/de-allocated in PageFile)");
	if ((freeFrames_before - freeFrames_after) != 160) kpanic_into_prompt("Wrong allocation: pages are not loaded successfully into memory %d", (freeFrames_before - freeFrames_after));
	...
	//Checking permissions on the allocated spaces
	{
		...
		while(1)
		{
			if (CB(ptr_page_directory, a, 0)!=1 || CB(ptr_page_directory, a, 1)!=1 || CB(ptr_page_directory, a, 2)!=0)
				kpanic_into_prompt("Wrong initialize: Wrong permissions - pages mapped with wrong permissions. Check them again.");
